...
func setupPkiApprolePrompt(logger *log.Logger) promptDef {
	prompt := mcp.NewPrompt("setup_pki_approle",
		mcp.WithPromptDescription("Provision a least-privilege AppRole that lets an application issue or sign certificates against a single Vault PKI role. Creates a scoped ACL policy, an AppRole role, and delivers the credentials response-wrapped."),
		...
		mcp.WithArgument("pki_mount", mcp.RequiredArgument(), mcp.ArgumentDescription("The PKI secrets engine mount (e.g. 'pki_int').")),
		mcp.WithArgument("pki_role", mcp.RequiredArgument(), mcp.ArgumentDescription("The PKI role to grant access to (e.g. 'example-dot-com').")),
		mcp.WithArgument("action", mcp.ArgumentDescription("Either 'issue' (Vault generates the key+cert) or 'sign' (sign a caller-supplied CSR). Defaults to 'issue'.")),
	...
	handler := func(_ context.Context, req mcp.GetPromptRequest) (*mcp.GetPromptResult, error) {
		if err := requireArgs(req, "app_name", "pki_mount", "pki_role"); err != nil {
			return nil, err
		}

		appName := arg(req, "app_name", "")
		pkiMount := arg(req, "pki_mount", "")
		pkiRole := arg(req, "pki_role", "")
		action := arg(req, "action", "issue")
		if action != "issue" && action != "sign" {
			return nil, fmt.Errorf("argument 'action' must be 'issue' or 'sign', got %q", action)
		}
		...
		policyName := fmt.Sprintf("%s-pki-%s", appName, pkiRole)
		grantedPath := fmt.Sprintf("%s/%s/%s", pkiMount, action, pkiRole)

		logger.WithFields(log.Fields{"prompt": "setup_pki_approle", "app": appName}).Debug("Rendering prompt")

		instruction := fmt.Sprintf(`You are provisioning a least-privilege Vault AppRole so the application %q can %s certificates against the PKI role %q on mount %q. Use the vault MCP tools and follow these steps exactly. Stop and report clearly if any verification step fails — do not improvise broader access.
...
1. Verify prerequisites:
   a. Call read_pki_role with mount=%q name=%q. If it does not exist, STOP and report that the PKI role must be created first.
...
6. Report back to the operator:
   ...
   - a one-line summary: "%s may %s certs via %s only".`,
			appName, action, pkiRole, pkiMount,
			...
			appName, policyName,
			appName, action, grantedPath,
		)

		return userMessage(fmt.Sprintf("Set up a least-privilege PKI signing AppRole for %s", appName), instruction), nil
	}
